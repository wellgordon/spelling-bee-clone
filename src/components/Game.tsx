...
import { WordList, wordLists } from '../constants/words';
...
const Game = () => {
  ...
  useEffect(() => {
    ...
    setActiveWordList(wordLists[index]);
  }, []);
  ...
  const handleSubmit = () => {
    if (bufferText.length < 4) {
      ...
      if (activeWordList?.words.includes(bufferText.toLowerCase())) {
  ...
  return (
    <GameUI
      ...
      letters={activeWordList?.letters}
    />
  );
};
